...
'''
    ...
    ################
        def nonLinAct(self, x):
        #return np.array(self.__a * (1 / (1 + np.exp(-self.__b * x)) * 2 - 1))
        #return np.array(self.__a * np.tanh(self.__b * x))
        return 1 - np.tanh(x) ** 2

    def nonLinActDer(self, x):
        #sig = 1 / (1 + np.exp(-self.__b * x))  # Стандартный sigmoid
        #return np.array(self.__b * sig * (1 - sig) * 5)  # Модифицированная производная
        return np.array(self.__b / self.__a * \
                        (self.__a - self.nonLinAct(x)) * \
                        (self.__a + self.nonLinAct(x)))
    ###################
